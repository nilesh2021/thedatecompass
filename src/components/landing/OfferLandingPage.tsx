...
export default function OfferLandingPage({ data }: { data: any }) {
  ...
  function VisitCta({ placement }: { placement: string }) {
    return (
      ...
      >
        ...
        <span className="flex w-12 shrink-0 items-center justify-center bg-brand-rose text-cream">→</span>
      </a>
    );
  }
      ...
      <main className="relative bg-ink font-display text-cream">
          ...
          <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:py-28">
            <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
              <div className="order-2 lg:order-1">
                <p className="tdc-eyebrow-mint mb-3">{data.category}</p>
                ...
                <div className="mt-8 flex items-center gap-4">
                  <VisitCta placement={`${data.slug}_hero`} />
                  <Link
                    href="#features"
        ...
        <section className="px-4 py-14 sm:px-6 lg:py-20">
          <div className="mx-auto max-w-6xl">
            <h2 className="tdc-eyebrow mb-4">Visual showcase</h2>
            <p className="max-w-2xl text-sm leading-relaxed text-fog">
              A curated selection of photos representing the listing and key visuals.
            </p>

            <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {[data.visuals.portrait, data.visuals.wide, data.visuals.hero].map((src, i) => (
                <div key={i} className="overflow-hidden rounded-2xl shadow-lg">
                  <div className="relative aspect-[4/5]">
                    <Image src={src} alt={`visual-${i}`} fill sizes="(max-width: 1024px) 50vw, 25vw" className="object-cover" />
                  </div>
                  <div className="p-3">
                    <h3 className="font-bold">{['Profiles','Messaging','Mobile'][i] || 'Highlight'}</h3>
                    <p className="mt-2 text-xs text-fog">{['See how profiles appear on the listing.','Messaging tools and contact flows.','Use the service on mobile devices.'][i] || 'Visual highlight'}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section id="about" className="px-4 py-12 sm:px-6 lg:py-20">
          <div className="mx-auto max-w-6xl">
            <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
              <div>
                <h2 className="tdc-eyebrow mb-4">About this listing</h2>
                <p className="text-2xl font-extrabold">What to expect</p>
                <p className="mt-4 max-w-xl text-sm leading-relaxed text-fog">{data.description}</p>
                <ul className="mt-6 space-y-3">
                  {data.features.slice(0,3).map((f:any,i:number)=> (
                    <li key={i} className="flex items-start gap-3">
                      <div className="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-brand-rose/10 text-brand-rose font-bold">{i+1}</div>
                      <div>
                        <p className="font-semibold">{f.title}</p>
                        <p className="mt-1 text-sm text-fog">{f.text}</p>
                      </div>
                    </li>
                  ))}
                </ul>
              ...
              <div className="mx-auto w-full">
                <div className="relative aspect-[3/4] w-full overflow-hidden rounded-3xl shadow-[0_24px_60px_rgba(0,0,0,0.45)]">
                  <Image src={data.visuals.portrait} alt="about" fill className="object-cover object-center" />
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="px-4 py-12 sm:px-6 lg:py-20">
          <div className="mx-auto max-w-6xl">
            <h2 className="tdc-eyebrow mb-4">Key features</h2>
            <p className="max-w-2xl text-sm leading-relaxed text-fog">Important features highlighted simply.</p>

            <div className="mt-8 grid gap-6 sm:grid-cols-3">
              {data.features.map((f:any,i:number)=> (
                <div key={i} className="rounded-2xl bg-ink-soft/60 p-6">
                  <h3 className="font-bold text-lg">{f.title}</h3>
                  <p className="mt-2 text-sm text-fog">{f.text}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="px-4 py-12 sm:px-6 lg:py-20">
          <div className="mx-auto max-w-6xl">
            <h2 className="tdc-eyebrow mb-4">Who it may suit</h2>
            <div className="mt-6 grid gap-6 sm:grid-cols-3">
              {data.audience.map((a:any,i:number)=> (
                <div key={i} className="rounded-2xl border border-cream/8 bg-ink-soft p-6">
                  <h3 className="font-bold">{a.title}</h3>
                  <p className="mt-2 text-sm text-fog">{a.text}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="px-4 py-12 sm:px-6 lg:py-20">
          <div className="mx-auto max-w-6xl">
            <h2 className="tdc-eyebrow mb-4">What to consider</h2>
            <div className="mt-6 grid gap-6 sm:grid-cols-3">
              {data.considerations.map((c:any,i:number)=> (
                <div key={i} className="rounded-2xl border border-cream/10 bg-ink p-6">
        ...
        <CountryFaqSection eyebrow="FAQ" title="Common questions" variant="germany" items={data.faqs} />

        <section className="px-4 pb-24 pt-12 sm:px-6 lg:pb-32 lg:pt-20">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl font-extrabold">Ready to visit?</h2>
            <p className="mt-4 text-sm text-fog">Open the destination site to review profiles, messaging, and current terms. Adults 18+ only.</p>
            <div className="mt-8 flex justify-center">
              <VisitCta placement={`${data.slug}_final_cta`} />
            </div>
